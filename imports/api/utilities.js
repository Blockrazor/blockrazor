...
export var removeUserCredit = (amount, userId, reason, rewardType) => {
  // if we need to remove user's credit for whatever reason
  if (Meteor.isServer) {
    UserData.upsert({ _id: userId }, { $inc: { balance: -amount } });
    ...
    Wallet.insert({
      ...
      message: `${formatAmount(
        amount
      )} KZR has been deducted from your account for ${reason}`,
      amount: formatAmount(-amount),
      read: false,
      rewardType: rewardType
    });
...
const quality = currency => {
  ...
  return (
    ((currency.eloRanking || 0) - eloMinElo) / (eloMaxElo - eloMinElo || 1)
  );
};

const intersection = (point, bound) =>
  point.y > bound.top &&
...
export class LocalizableCollection extends Mongo.Collection {
  ...
  populateLocal() {
    ...
    var observeHandle = super.find().observeChanges(observer);
    if (!this.populating) {
      ...
      if (!this.ready) {
        ...
        if (Tracker.active) {
          ...
          let observer = {
            ...
            added(id, fields) {
              self.local.upsert(id, {
                $set: Object.assign({ _id: id }, fields)
              });
            },
            removed(id) {
