...
if(Meteor.isServer) {
  ...
  Meteor.methods({
    ...
    completeCurrencyBounty: (currencyName) => {
      Bounties.update({
        type: 'new-currency',
        userId: Meteor.userId(),
        completed: false
    ...
    deleteCurrencyBounty: (id, token) => {
      if (token === 's3rver-only')
        Bounties.remove({
          _id: id
        })
    },
    deleteHashPowerBounty: (id, token) => {
      if (token === 's3rver-only')
        Bounties.remove({
          _id: id
        })
    },
    deleteCurrencyBountyClient: () => {
      Bounties.remove({
        userId: Meteor.userId(),
        type: 'new-currency',
        completed: false
  ...
  Meteor.publish('currencyBounty', userId => Bounties.find({
    userId: Meteor.userId(),
    type: 'new-currency'
  }))
  Meteor.publish('hashPowerBounty', userId => Bounties.find({
    userId: Meteor.userId(),
    type: 'new-hashpower'
  }))
