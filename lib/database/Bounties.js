...
if(Meteor.isServer) {
  ...
  Meteor.methods({
    ...
    startBounty: function(id){
      ...
      if(Meteor.user() && !UserData.findOne({_id: Meteor.userId()}).workingOnBounty) {
      Bounties.upsert(id, {
        $set: {
...
var fetchHashrate = function(id) {
  ...
  HTTP.get(Bounties.findOne({_id: id}).APICall, (error, response) => { //Bounties.findOne({_id: id}).APICall
    if(!error) {
      ...
      console.log(eval(Bounties.findOne({_id: id}).Hashrate));
    }
  });
}
